 ...
 * payer's data it echoes back) into our API and, through the idempotency store, replayed it for 24
 * h. The bank's text stays where support looks: {@code provider_requests} (ProviderGateway) and one
  ...
  private static final Map<String, String> MESSAGES =
      Map.of(
          ...
          "PROVIDER_UNAVAILABLE",
              "The bank could not process the request right now. Try again later.",
          "PROVIDER_TIMEOUT", "The bank did not answer in time.");

  private ProviderErrors() {}

  public static DomainException toDomain(
      String code, ProviderException cause, Logger log, String operation, String resourceId) {
    log.warn(
        "{} for {} failed at the bank: {} {} {}",
        operation,
        resourceId,
        cause.code(),
        cause.providerType(),
