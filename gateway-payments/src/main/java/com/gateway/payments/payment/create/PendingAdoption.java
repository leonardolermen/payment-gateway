...
/**
 * CREATED -> PENDING, with the bank's side of the charge, the jobs it needs and the outbox row — all in
 * one transaction.
 *
 * <p>Idempotent on purpose: a payment no longer CREATED is returned as it is. The stuck-CREATED sweeper
 * and a slow create can race to adopt the same charge, and the loser must not fail on PENDING ->
 * PENDING.
 *
...
public class PendingAdoption {
  ...
  public Payment adoptPix(String paymentId, Charge accepted, int fallbackExpires, EventSource by) {
    return unitOfWork.inTransaction(() -> {
      ...
      if (accepted.txid() != null && !accepted.txid().equals(payment.id())) {
        log.warn("bank echoed txid {} for payment {}; keeping ours", accepted.txid(), payment.id());
      }
  ...
  /** With both sides, the expire job (limit date's end of day + grace), the poll job and the outbox row. */
  ...
  /**
   * {@code unconfirmedDetail} non-null opens PIX_TXID_UNCONFIRMED in the same transaction, and only if
   * this call made the transition.
   */
  public Payment adoptBolecode(String paymentId, IssuedBoleto issued, EventSource by, String unconfirmedDetail) {
    return unitOfWork.inTransaction(() -> {
      Payment payment = payments.findById(paymentId).orElseThrow();
      if (payment.status() != PaymentStatus.CREATED) {
      ...
      Payment saved =
          payments.save(
              payment,
              List.of(payment.markPendingBolecode(pix, details, BoletoDates.endOfDay(details.paymentLimitDate()), by)));
