...
/**
 * The CREATED row, written before the bank is ever called: a timeout can then never lose a charge the
 * bank accepted, because there is always a row to reconcile against.
 *
 * <p>For a bolecode the nosso número is reserved in the same transaction as the row (spec 2026-09-25
 * §7). Two transactions would let a crash between them burn a number, and a number the bank already
 * knows cannot be reused for 45 days.
 */
public class PaymentDraftFactory {
  ...
  public Payment bolecode(
      CreateBolecodePayment command, String providerId, Payer payer, LocalDate dueDate, LocalDate paymentLimitDate) {
    return unitOfWork.inTransaction(() -> {
      String nossoNumero = boletoNumbers.next(command.merchantId());
      BoletoDetails details = new BoletoDetails(nossoNumero, null, null, null, dueDate, paymentLimitDate, null);
