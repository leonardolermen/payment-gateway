...
interface PaymentJpaRepository extends JpaRepository<PaymentEntity, String> {
  ...
  @Query(
      "SELECT p FROM PaymentEntity p WHERE p.status = 'PENDING' AND p.expiresAt < :before ORDER BY p.expiresAt ASC")
  ...
  @Query(
      "SELECT p FROM PaymentEntity p WHERE p.status = :status AND p.createdAt < :before ORDER BY p.createdAt ASC")
  ...
  @Query(
      "SELECT p FROM PaymentEntity p WHERE p.method = :method AND p.status IN :statuses AND p.createdAt > :after ORDER BY p.createdAt ASC")
  ...
  @Query(
      "SELECT p FROM PaymentEntity p WHERE p.merchantId = :merchantId AND (:cursorId IS NULL OR p.id < :cursorId) ORDER BY p.id DESC")
  java.util.List<PaymentEntity> findByMerchant(
      @Param("merchantId") String merchantId, @Param("cursorId") String cursorId, Limit limit);
  ...
  @Query(
      value =
          "SELECT * FROM payments.payments WHERE provider = :provider AND details->'pix'->>'txid' = :txid",
      nativeQuery = true)
   ...
   *
   * <p>{@code clearAutomatically = true} matters within a single transaction: a bulk JPQL {@code
   * UPDATE} bypasses the persistence context, so without it a later {@code findById} in the same
   * transaction would return the stale cached entity from before this write, with the old {@code
   * version} — the next {@code save} would then compute a stale {@code expectedVersion} and fail
   * with a false optimistic-lock error even though nobody else touched the row. {@code
   * flushAutomatically = true} makes sure any pending changes are flushed before this bulk update
   * runs, so it never overwrites something not yet written.
   */
