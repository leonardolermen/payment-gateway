...
/**
 * Creating a bolecode — a registered boleto with a Pix QR on the same issue. Spec 2026-09-25 §7. Order
 * matters: the credentials (beneficiary included) and the payer are checked before a row exists; the
 * number is reserved in the same transaction as CREATED; the bank call runs outside any transaction;
 * PENDING lands with both sides and both jobs.
 */
public class BolecodePaymentFlow implements PaymentFlow {
  ...
  /**
   * CONFLICT joins the two network codes: the bank saying "this nosso número already exists" (ruling R4)
   * can only be our own earlier attempt that landed, because the number is ours and was reserved before
   * the call. Failing it would leave a payable boleto behind a FAILED payment.
   */
  ...
  public Payment create(CreatePaymentCommand command) {
    ...
    ResolvedProvider<BoletoMethodProvider> resolved =
        providers.resolveBoleto(bolecode.merchantId(), bolecode.environment(), PaymentService.PROVIDER);
    ...
    Payment payment = drafts.bolecode(bolecode, PaymentService.PROVIDER, payer, dueDate, paymentLimitDate);
  ...
  private int paymentLimitDaysOf(CreateBolecodePayment bolecode) {
    int limitDays =
        bolecode.paymentLimitDays() == null ? props.boletoDefaultPaymentLimitDays() : bolecode.paymentLimitDays();

    if (limitDays < 0 || limitDays > props.boletoMaxPaymentLimitDays()) {
      throw new DomainException(
          "INVALID_PAYMENT_LIMIT", "payment_limit_days must be between 0 and " + props.boletoMaxPaymentLimitDays());
    }
   ...
   *
   * <p>Unlike Pix, it is NOT failed on the spot: the bank's 202 means "operação em andamento", so an
   * empty query a second later proves nothing. {@code ExpirationService.sweepStuckCreated} asks again
   * after {@code stuckCreatedAfter} and decides then.
   */
  private Payment recover(
    ...
    if (existing.isEmpty()) {
      log.warn(
          ...
          nossoNumero, payment.id(), failure.code());
    ...
    try {
      ...
      // The boleto exists but GET /cob itself failed (bank unreachable), so nothing is known about the
      // txid yet. This is not the "bank does not know the txid" case: that one is adopted and flagged
      // PIX_TXID_UNCONFIRMED inside BolecodeFromQuery (ruling R1). Here the payment stays CREATED and the
      // sweeper asks again after stuckCreatedAfter. Translated here, not inside BolecodeFromQuery, so the
