 ...
 *
 * <p>Which codes may mean "it landed" is the flow's call and arrives as a set, not as a flag on this
 * class: for Pix it is TIMEOUT and UNAVAILABLE, because neither says whether the PUT reached the bank;
 * for a boleto CONFLICT joins them, because the nosso número is ours and was reserved before the call,
 * so "this number already exists" can only be our own earlier attempt that arrived (ruling R4).
 */
public final class ProviderFailures {
  ...
  /**
   * The merchant-facing code for a call that may have landed but could not be confirmed. A timeout is
   * told apart from the rest because it is the one a client is expected to retry with the same
   * Idempotency-Key.
   */
