...
/**
 * Adopting a boleto the bank has but we never got the answer for: the issue's response was lost, so
 * the query carries the boleto's identity while the Pix side is still unknown.
 *
 * <p>The txid follows the bank's documented formula ({@code BoletoMethodProvider.pixTxidFor}) and
 * is confirmed with GET /cob/{txid}, which also yields the EMV. If the bank does not know that
 * txid, the query's own {@code qrcode_pix.emv} is used and a divergence records that the formula
 * did not match — the payment is still ADOPTED (ruling R1). Refusing it would leave a boleto the
 * bank issued sitting in CREATED, and the sweeper would later fail it while it is still payable.
 */
  ...
  public Payment adopt(
    ...
    Optional<Charge> charge =
        providers.call(
            ...
            "findCharge",
            pixSide,
            target -> target.provider().find(target.credentials(), txid));
