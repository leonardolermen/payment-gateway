...
public class InMemoryCredentialLookup implements CredentialLookup {
  ...
  public Optional<ProviderCredentials> find(MerchantId merchantId, String provider, ProviderEnvironment env) {
    if ("ITAU".equals(provider) && env == ProviderEnvironment.TEST) {
      return Optional.of(new ProviderCredentials(("{\"client_id\":\"test\",\"beneficiary_id\":\"" + beneficiaryOf(merchantId) + "\"}").getBytes(StandardCharsets.UTF_8), env));
    }
  ...
  public static String beneficiaryOf(MerchantId merchantId) {
    return String.format("%012d", Math.floorMod(merchantId.value().hashCode() * 2654435761L, 1_000_000_000_000L));
  }
}
