...
class BolecodeLifecycleIntegrationTest extends ServiceIntegrationTestBase {
  ...
  private java.util.List<String> divergences(String paymentId) {
    return jdbc.queryForList(
        "SELECT provider_status FROM payments.reconciliation_divergences WHERE payment_id = ? ORDER BY created_at",
        String.class,
        paymentId);
  }
  ...
  void expirationAfterTheLimitDateAsksTheBankAndExpiresAnOpenBoletoWithoutABaixa() {
    ...
    assertThat(boletos.callsFor(merchant, nn(p)))
        .containsExactly("issueBoleto:" + nn(p), "findBoleto:" + nn(p));
  ...
  void aLatePaymentAfterExpiryCompletesThroughThePoll() {
    ...
    // The reconciliation's boleto pass looks at PENDING, CANCELED and FAILED, not EXPIRED; the poll
    // job (still within limit + 2 days) is what completes an EXPIRED one.
    assertThat(payments.findById(p.id()).orElseThrow().status()).isEqualTo(PaymentStatus.EXPIRED);
    assertThat(polling.check(p.id(), com.gateway.payments.payment.EventSource.PROVIDER_POLL))
        .isTrue();
    Payment done = payments.findById(p.id()).orElseThrow();
    assertThat(done.status()).isEqualTo(PaymentStatus.COMPLETED);
    assertThat(payments.events(p.id()).getLast().source())
        .isEqualTo(com.gateway.payments.payment.EventSource.PROVIDER_POLL);
  }
  ...
  void reconciliationChecksPendingBolecodesOlderThanMinAge() {
    ...
    boletos.markPaid(nn(p), Money.brl(12990), clock.instant());
    reconciliation.reconcileAll(clock.instant());
    assertThat(payments.findById(p.id()).orElseThrow().status())
        .as("too young")
    ...
    assertThat(done.status()).isEqualTo(PaymentStatus.COMPLETED);
    assertThat(payments.events(p.id()).getLast().source())
        .isEqualTo(com.gateway.payments.payment.EventSource.RECONCILIATION);
  }
  ...
  void theBoletoPassQueriesBolecodesOnlySoOldPixRowsCannotFillItsCap() {
    ...
    var rows =
        payments.findByMethodAndStatusIn(
            com.gateway.kernel.payment.PaymentMethod.BOLECODE,
            java.util.EnumSet.of(PaymentStatus.PENDING),
            clock.instant().minus(Duration.ofHours(1)),
            1000);
    assertThat(rows)
        .extracting(Payment::method)
        .containsOnly(com.gateway.kernel.payment.PaymentMethod.BOLECODE);
  ...
  void aPixWebhookOnABolecodeIsMatchedByTxidAndCompletesWithPaidViaPix() {
    ...
    String inboxId =
        webhookInbox.accept(
            ...
            merchant,
            "{}",
            ("E2E-QR-" + nn(p) + " " + txid + " 12990").getBytes(StandardCharsets.UTF_8));
    ...
    assertThat(
            jdbc.queryForObject(
                "SELECT status FROM payments.webhook_inbox WHERE id = ?", String.class, inboxId))
        .isEqualTo("PROCESSED");
  ...
  void aRefundOfABoletoSettlementIsRefused() {
    ...
    paymentService.settleBoleto(
        ...
        p.id(),
        boletos.status(nn(p)),
        com.gateway.payments.payment.EventSource.PROVIDER_POLL);
    assertThatThrownBy(() -> refunds.request(merchant, p.id(), Money.brl(1000)))
        .isInstanceOfSatisfying(
            DomainException.class, e -> assertThat(e.code()).isEqualTo("REFUND_NOT_SUPPORTED"));
    assertThat(bank.callsFor(p.id())).noneMatch(c -> c.startsWith("requestRefund"));
    assertThat(
            jdbc.queryForObject(
                "SELECT count(*) FROM payments.refunds WHERE payment_id = ?", Long.class, p.id()))
        .isZero();
  }
  ...
  void reconciliationSeesABarcodePaidAfterTheBaixaAsBoletoPaidWithoutGrowingTheLog() {
    ...
    assertThat(payments.events(p.id()))
        .extracting(e -> e.type())
        .containsExactly("created", "pending", "canceled");
  }
  ...
  private void pixWebhook(Payment p, String e2e) {
    String inboxId =
        webhookInbox.accept(
            ...
            merchant,
            "{}",
            (e2e + " " + p.pix().txid() + " 12990").getBytes(StandardCharsets.UTF_8));
    webhookInbox.process(inboxId);
  }
