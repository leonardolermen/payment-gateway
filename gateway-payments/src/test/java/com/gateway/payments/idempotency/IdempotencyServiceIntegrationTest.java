...
class IdempotencyServiceIntegrationTest extends ServiceIntegrationTestBase {
  ...
  void firstProceedsThenInProgressThenReplays() {
    ...
    assertThat(idempotency.begin(merchant, "key-1", hash))
        .isInstanceOf(IdempotencyService.Outcome.InProgress.class);

    idempotency.finish(
        ((IdempotencyService.Outcome.Proceed) first).k(), 201, "{\"id\":\"x\"}", null);

    assertThat(idempotency.begin(merchant, "key-1", hash))
        .isEqualTo(
            new IdempotencyService.Outcome.Replayed(
                new IdempotencyService.Replay(201, "{\"id\":\"x\"}")));
  }
