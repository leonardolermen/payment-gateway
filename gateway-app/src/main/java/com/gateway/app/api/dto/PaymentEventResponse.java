...
public record PaymentEventResponse(String id, long sequence, String type, String source, @JsonRawValue String payload, Instant at) {

  public static PaymentEventResponse from(PaymentEvent e) {
    return new PaymentEventResponse(e.id(), e.sequence(), e.type(), e.source().name(), e.payload(), e.at());
  }
}
