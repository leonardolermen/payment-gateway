...
/**
 * A Pix charge. {@code expires_in} is the field that is Pix's alone; {@code customer} is optional
 * and only its document is used, to restrict the QR to whoever was charged.
 *
 * <p>A field belonging to the other method is refused by the deserialiser, not by an if: the mapper
 * has FAIL_ON_UNKNOWN_PROPERTIES on (application.yml), because silently swallowing a due_date sent
 * here is the very mistake this shape exists to catch.
 */
