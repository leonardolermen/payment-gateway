 ...
 * copy; if the two ever drifted, {@code RateLimitFilter} could run on a route {@code ApiKeyAuthFilter} had not authenticated and call
 * {@code MerchantContext.current()} with nothing set — an {@code UnauthenticatedException} thrown from
...
final class ProtectedRoutes {
  ...
  /** {@code path} is always {@link RequestPath#normalized()}: the path MVC routes on, never the raw URI. */
