...
/**
 * The path the filters classify on must be the path Spring MVC routes on. Raw {@code
 * getRequestURI()} is not: MVC decodes percent-encoding and drops {@code ;matrix} content, so
 * {@code /v1/%61dmin/merchants} and {@code /v1/admin;x/merchants} reached the admin controller
 * while {@code AdminKeyFilter} skipped them and {@code ApiKeyAuthFilter} accepted a merchant key.
 */
