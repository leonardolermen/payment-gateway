...
public class JobScheduler {
  ...
  /**
   * Batches per tick: 20 x 20 jobs. A backlog beyond that waits one poll interval instead of
   * pinning this thread.
   */
  ...
  /**
   * Idempotent (ON CONFLICT DO NOTHING); the cron only re-creates the singleton if it ever
   * vanished.
   */
  @Scheduled(cron = "0 */15 * * * *")
  ...
  /**
   * The safety net under the per-payment EXPIRE_PAYMENT job: that job is the normal path, this
   * sweep catches a PENDING charge whose job went DEAD or was never enqueued.
   */
