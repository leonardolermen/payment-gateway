 ...
 * (paid_via BOLETO, webhook delivered), and the refund refusal. WireMock plays the three Itaú APIs
 * under one host with three path prefixes; sandbox-shaped credentials, no mTLS.
 *
 * <p>One test method on purpose, like PaymentsFlowIntegrationTest: every step builds on the
 * previous.
 */
@SpringBootTest(
...
class BolecodeFlowIntegrationTest {
  ...
  static void itau(DynamicPropertyRegistry r) {
    ...
    r.add(
        "gateway.providers.itau.boleto.test-issue-token-url",
        () -> ITAU.baseUrl() + "/api/oauth/jwt");
    r.add("gateway.providers.itau.boleto.test-query-api-base", () -> ITAU.baseUrl() + "/query");
    r.add(
        "gateway.providers.itau.boleto.test-query-token-url",
        () -> ITAU.baseUrl() + "/api/oauth/jwt");
    r.add(
        "gateway.providers.itau.boleto.test-instruction-api-base",
        () -> ITAU.baseUrl() + "/instruction");
    r.add(
        "gateway.providers.itau.boleto.test-instruction-token-url",
        () -> ITAU.baseUrl() + "/api/oauth/jwt");
  }
  ...
  static void start() throws IOException {
    ...
    ITAU.stubFor(
        post(urlEqualTo("/issue/boletos-pix"))
            .willReturn(
                aResponse()
                    .withStatus(200)
                    .withHeader("Content-Type", "application/json")
                    .withBody(fixture("post_boletos_pix_200.json"))));
  }
  ...
  private static String fixture(String name) {
    try (InputStream in =
        BolecodeFlowIntegrationTest.class.getResourceAsStream("/itau/fixtures/" + name)) {
  ...
  private EntityExchangeResult<Map> postPayment(
      String apiKey, String key, Map<String, Object> body) {
    return http()
        .post()
        .uri("/v1/payments")
  ...
  void issueRefuseCancelPollAndRefuseRefund() {
    ...
    String merchantId =
        (String) adminPost("/v1/admin/merchants", Map.of("name", "Boleto Store")).get("id");
    String testKey =
        (String)
            adminPost(
    ...
    assertThat(payment)
        ...
        .containsEntry("method", "BOLECODE")
        .containsEntry("amount", 12990)
        .containsKey("expires_at");
    Map<String, Object> boleto = (Map<String, Object>) payment.get("boleto");
    assertThat(boleto)
        .containsEntry("linha_digitavel", "34101234567890123456789012345678901234567890123")
        .containsEntry("codigo_barras", "34191234567890123456789012345678901234567890")
        .containsEntry("due_date", today.plusDays(3).toString())
        .containsEntry(
            "payment_limit_date", "2027-01-31") // the bank's answer wins over our due + 30
        .containsEntry("paid_via", null);
    Map<String, Object> pix = (Map<String, Object>) payment.get("pix");
    assertThat(pix)
        .containsEntry("txid", "BL1234567890123456789012345678901")
        .containsEntry("end_to_end_id", null);
    assertThat((String) pix.get("copia_e_cola")).startsWith("000201");
    ITAU.verify(
        1,
        postRequestedFor(urlEqualTo("/issue/boletos-pix"))
            .withHeader("Authorization", equalTo("Bearer tok-123"))
            .withRequestBody(
                matchingJsonPath("$.beneficiario.id_beneficiario", equalTo("150000052061")))
            .withRequestBody(
                matchingJsonPath(
                    "$.dado_boleto.pagador.pessoa.nome_pessoa", equalTo("Joao da Silva")))
            .withRequestBody(
                matchingJsonPath(
                    "$.dado_boleto.dados_individuais_boleto[0].numero_nosso_numero",
                    equalTo("00000001")))
            .withRequestBody(
                matchingJsonPath(
                    "$.dado_boleto.dados_individuais_boleto[0].valor_titulo", equalTo("129.90")))
            .withRequestBody(
                matchingJsonPath(
                    "$.dado_boleto.dados_individuais_boleto[0].data_vencimento",
                    equalTo(today.plusDays(3).toString()))));
    Awaitility.await()
        .atMost(Duration.ofSeconds(15))
        .until(
            () ->
                received.stream()
                    .anyMatch(r -> r.startsWith("payment.pending") && r.contains(paymentId)));
    ...
    noZip.put(
        ...
        Map.of(
            ...
            "12345678901",
            "address",
            Map.of("street", "Rua A", "district", "Centro", "city", "Sao Paulo", "state", "SP")));
    EntityExchangeResult<Map> refusedZip = postPayment(testKey, "b3", noZip);
    assertThat(refusedZip.getStatus().value()).isEqualTo(422);
    assertThat((String) refusedZip.getResponseBody().get("detail"))
        .contains("customer.address.zip");
    ...
    ITAU.stubFor(
        get(urlPathEqualTo("/query/boletos"))
            .withQueryParam("nosso_numero", equalTo("00000001"))
            .willReturn(
                aResponse()
                    .withStatus(200)
                    .withHeader("Content-Type", "application/json")
                    .withBody(
                        fixture("get_boletos_200_canceled.json")
                            .replace("\"Baixado\"", "\"Em Aberto\""))));
    ITAU.stubFor(
        patch(urlEqualTo("/instruction/boletos/" + BAIXA_ID_1 + "/baixa"))
    ...
    ITAU.verify(
        1,
        patchRequestedFor(urlEqualTo("/instruction/boletos/" + BAIXA_ID_1 + "/baixa"))
            .withHeader("Authorization", equalTo("Bearer tok-123")));

    // 5. Second Bolecode (a different txid from the bank, or the unique index refuses it), paid by
    // barcode and found by the poll.
    ITAU.stubFor(
        post(urlEqualTo("/issue/boletos-pix"))
            .willReturn(
                aResponse()
                    ...
                    .withHeader("Content-Type", "application/json")
                    .withBody(
                        fixture("post_boletos_pix_200.json")
                            .replace(
                                "BL1234567890123456789012345678901",
                                "BL1234567890123456789012345678902"))));
    ...
    ITAU.stubFor(
        get(urlPathEqualTo("/query/boletos"))
            .withQueryParam("nosso_numero", equalTo("00000002"))
            .willReturn(
                aResponse()
                    .withStatus(200)
                    .withHeader("Content-Type", "application/json")
                    .withBody(
                        fixture("get_boletos_200_paid.json")
                            .replace("\"00000001\"", "\"00000002\"")
                            .replace("\"2100.00\"", "\"129.90\""))));
    // The poll is due in 6 hours; bring it forward instead of waiting.
    jdbc.update(
        "UPDATE payments.jobs SET next_run_at = now() WHERE type = 'POLL_BOLETO' AND ref_id = ?",
        secondId);
    Awaitility.await()
        .atMost(Duration.ofSeconds(15))
        .until(
            () -> "COMPLETED".equals(getJson(testKey, "/v1/payments/" + secondId).get("status")));
    ...
    Awaitility.await()
        ...
        .until(
            () ->
                received.stream()
                    .anyMatch(r -> r.startsWith("payment.completed") && r.contains(secondId)));
    assertThat(
            received.stream()
                .filter(r -> r.startsWith("payment.completed") && r.contains(secondId))
                .findFirst()
                .orElseThrow())
    ...
    List<Map<String, Object>> events =
        http()
            .get()
            .uri("/v1/payments/" + secondId + "/events")
            .header("Authorization", "Bearer " + testKey)
            .exchange()
            .expectStatus()
            .isOk()
    ...
    EntityExchangeResult<Map> refund =
        http()
            .post()
            .uri("/v1/payments/" + secondId + "/refunds")
            .header("Authorization", "Bearer " + testKey)
    ...
    Map<String, Object> pixWithDue =
        new HashMap<>(
            Map.of("amount", 100, "currency", "BRL", "method", "PIX", "due_date", "2026-12-31"));
