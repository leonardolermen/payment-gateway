...
@SpringBootTest(
    ...
    properties = {
      ...
      // The test profile's 5/min would trip halfway through this story; the limiter has its own test.
...
class PaymentsFlowIntegrationTest {
  ...
  static void start() throws IOException {
    ...
    ITAU.stubFor(put(urlMatching("/pix/[A-Za-z0-9]+/devolucao/[A-Za-z0-9]+"))
  ...
  void chargeReplaySettleAndRefundAgainstItau() {
    ...
    String merchantId = (String) adminPost("/v1/admin/merchants", Map.of("name", "Pix Store")).get("id");
    String testKey = (String) adminPost("/v1/admin/merchants/" + merchantId + "/api-keys", Map.of("environment", "TEST")).get("key");
    http().put().uri("/v1/admin/merchants/" + merchantId + "/providers/ITAU/credentials")
    ...
    http().post().uri("/v1/webhooks/endpoints").header("Authorization", "Bearer " + testKey).contentType(MediaType.APPLICATION_JSON)
        .body(Map.of("url", "http://localhost:" + sink.getAddress().getPort() + "/hook", "events", List.of("payment.*")))
        .exchange().expectStatus().isCreated();

    // 2. Create the charge.
    Map<String, Object> request = Map.of("amount", 15990, "currency", "BRL", "method", "PIX", "reference", "order-42", "description", "Pedido 42");
    ...
    assertThat(payment).containsEntry("status", "PENDING").containsEntry("amount", 15990).containsEntry("environment", "TEST")
        .containsEntry("reference", "order-42").containsKey("expires_at").containsKey("created_at");
    Map<String, Object> pix = (Map<String, Object>) payment.get("pix");
    assertThat(pix.get("copia_e_cola")).isEqualTo(new String(fixture("put_cob_201.json")).replaceAll("(?s).*\"pixCopiaECola\": \"([^\"]+)\".*", "$1"));
    assertThat(created.getResponseHeaders().getFirst("X-Resource-Id")).as("internal header must not leak").isNull();

    ITAU.verify(1, postRequestedFor(urlEqualTo("/api/oauth/jwt"))
        .withRequestBody(containing("grant_type=client_credentials")).withRequestBody(containing("client_id=sandbox-client")));
    ITAU.verify(1, putRequestedFor(urlEqualTo("/cob/" + paymentId))
    ...
    EntityExchangeResult<Map> reused = postPayment(testKey, "k1", Map.of("amount", 100, "currency", "BRL", "method", "PIX"));
    assertThat(reused.getStatus().value()).isEqualTo(422);
    assertThat(reused.getResponseBody()).containsEntry("type", "urn:gateway:IDEMPOTENCY_KEY_REUSED");
    ...
    String webhook = fixture("webhook_pix.json")
        ...
        // The fixture's 2020 payment time would put the refund below outside the bank's 90-day window.
    ...
    assertThat(((Map<String, Object>) completed.get("pix")).get("end_to_end_id")).isEqualTo("E12345678202009091221kkkkkkkkkkk");
    ...
    Map<String, Object> refund = http().post().uri("/v1/payments/" + paymentId + "/refunds").header("Authorization", "Bearer " + testKey)
        .header("Idempotency-Key", "r1").contentType(MediaType.APPLICATION_JSON).body(Map.of("amount", 5000))
        .exchange().expectStatus().isCreated().expectBody(Map.class).returnResult().getResponseBody();
    String refundId = (String) refund.get("id");
    assertThat(refund).containsEntry("state", "PROCESSING").containsEntry("amount", 5000).containsEntry("payment_id", paymentId);
    ITAU.verify(1, putRequestedFor(urlEqualTo("/pix/E12345678202009091221kkkkkkkkkkk/devolucao/" + refundId)));

    ITAU.stubFor(get(urlEqualTo("/pix/E12345678202009091221kkkkkkkkkkk/devolucao/" + refundId))
        .willReturn(aResponse().withStatus(200).withHeader("Content-Type", "application/json")
            .withBody(fixture("get_devolucao_200_done.json").replace("\"123456\"", "\"" + refundId + "\"").replace("\"7.89\"", "\"50.00\""))));
    // The poll is due in 5 minutes; bring it forward instead of waiting.
    jdbc.update("UPDATE payments.jobs SET next_run_at = now() WHERE type = 'POLL_REFUND' AND ref_id = ?", refundId);

    Awaitility.await().atMost(Duration.ofSeconds(15)).until(() -> "COMPLETED".equals(getJson(testKey, "/v1/refunds/" + refundId).get("state")));
    assertThat(getJson(testKey, "/v1/payments/" + paymentId)).containsEntry("refunded_amount", 5000);
    ...
    assertThat(noKey.getResponseBody()).containsEntry("type", "urn:gateway:IDEMPOTENCY_KEY_REQUIRED");

    // 9. LIVE key, no LIVE credential: refused before any bank call.
    String liveKey = (String) adminPost("/v1/admin/merchants/" + merchantId + "/api-keys", Map.of("environment", "LIVE")).get("key");
