...
class MerchantsIntegrationTest {
  ...
  void inboundWebhookTokenResolvesItsMerchantAndSurvivesSuspension() {
    ...
    assertThat(merchants.findByInboundWebhookToken(m.inboundWebhookToken()))
        .map(x -> x.id())
        .contains(m.id());
  ...
  void issuesAuthenticatesAndRotates() {
    ...
    ApiKey old =
        apiKeys.list(m.id()).stream()
            .filter(k -> k.id().equals(k1.apiKey().id()))
            .findFirst()
            .orElseThrow();
  ...
  void databaseHoldsNeitherPlainKeyNorPlainCredential() {
    ...
    ApiKey.Issued k = apiKeys.issue(m.id(), ApiKeyEnvironment.LIVE);
    byte[] cred =
        "{\"client_id\":\"abc123\",\"client_secret\":\"itau-secret\"}"
            .getBytes(StandardCharsets.UTF_8);
    credentials.store(m.id(), Provider.ITAU, ApiKeyEnvironment.LIVE, cred);

    String apiKeyDump =
        String.join(
            "|", jdbc.queryForList("SELECT hash || prefix FROM merchants.api_keys", String.class));
    assertThat(apiKeyDump).doesNotContain(k.plainKey().reveal());
    byte[] ciphertext =
        jdbc.queryForObject(
            "SELECT ciphertext FROM merchants.provider_credentials WHERE merchant_id = ?",
            byte[].class,
            m.id().value());
    assertThat(new String(ciphertext, StandardCharsets.ISO_8859_1))
        .doesNotContain("itau-secret")
  ...
  void storingAgainReplacesThePayload() {
    ...
    assertThat(credentials.decrypt(m.id(), Provider.ITAU, ApiKeyEnvironment.LIVE))
        .contains("v2".getBytes());
  }
  ...
  void expiredRotatedKeyNoLongerCountsTowardsTheCap() {
    ...
    jdbc.update(
        "UPDATE merchants.api_keys SET expires_at = now() - interval '1 hour' WHERE merchant_id = ? AND expires_at IS NOT NULL",
        m.id().value());
  ...
  void liveAndTestCiphertextsCannotBeSwapped() {
    ...
    jdbc.update(
        ...
        """,
        m.id().value());
    assertThatThrownBy(() -> credentials.decrypt(m.id(), Provider.ITAU, ApiKeyEnvironment.LIVE))
        .isInstanceOf(SecurityException.class);
    assertThatThrownBy(() -> credentials.decrypt(m.id(), Provider.ITAU, ApiKeyEnvironment.TEST))
        .isInstanceOf(SecurityException.class);
  }
}
