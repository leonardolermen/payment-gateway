...
public class MerchantService {
  ...
  public Merchant create(String name) {
    return repo.save(Merchant.create(name));
  }
  ...
  @Transactional
  public Merchant activate(MerchantId id) {
    return repo.save(get(id).activate());
  }

  /** Empty for an unknown token: the webhook answers 404 either way, so existence does not leak. */
  @Transactional(readOnly = true)
  public Optional<Merchant> findByInboundWebhookToken(String token) {
