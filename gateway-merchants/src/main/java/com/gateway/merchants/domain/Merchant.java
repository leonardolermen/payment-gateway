...
public record Merchant(MerchantId id, String name, MerchantStatus status, String inboundWebhookToken, Instant createdAt, Instant updatedAt) {
  public Merchant {
    ...
    if (inboundWebhookToken == null || inboundWebhookToken.length() != 26) throw new IllegalArgumentException("inbound webhook token must be 26 characters");
  }
  ...
  public Merchant suspend() { return new Merchant(id, name, MerchantStatus.SUSPENDED, inboundWebhookToken, createdAt, Instant.now()); }
  public Merchant activate() { return new Merchant(id, name, MerchantStatus.ACTIVE, inboundWebhookToken, createdAt, Instant.now()); }
