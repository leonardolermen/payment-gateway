...
import com.gateway.providers.itau.auth.ItauTokenClientMtlsTest;
...
class PixApiClientContractTest {
  ...
  static String fixture(String f) {
    try {
      return Files.readString(
          Path.of("src/test/resources/itau/fixtures/" + f), StandardCharsets.UTF_8);
    } catch (Exception e) {
  ...
  static ItauCredentials creds() {
    return ItauCredentials.parse(
        ("{\"client_id\":\"sandbox-client\",\"client_secret\":\"sandbox-secret\",\"x_itau_apikey\":\""
                + APIKEY
  ...
  @Test
  void putCobSendsHeadersAndBodyAndParses201() {
    server.stubFor(
        put("/v2/cob/" + TXID)
            ...
            // ignoreExtraElements: we also send calendario.expiracao, which the minimal example
            // omits
    ...
    CobResponse r =
        client()
            .putCob(
                creds(),
                TXID,
                CobRequest.forCharge(Money.brl(12345), 3600, "60701190000104", null, null, null));
  ...
  @Test
  void correlationIdComesFromMdcWhenItIsAUuid() {
    ...
    try {
      client().getCob(creds(), TXID);
    } finally {
      MDC.remove("correlationId");
    }
    server.verify(
        getRequestedFor(urlEqualTo("/v2/cob/" + TXID))
            .withHeader("x-itau-correlationID", equalTo("01234567-89ab-cdef-0123-456789abcdef")));
  }

  @Test
  void nonUuidCorrelationIdIsReplaced() {
    server.stubFor(get("/v2/cob/" + TXID).willReturn(okJson(fixture("get_cob_200_active.json"))));
  ...
  @Test
  void sandboxWithoutApiKeyOmitsTheHeader() {
    server.stubFor(get("/v2/cob/" + TXID).willReturn(okJson(fixture("get_cob_200_active.json"))));
    ItauCredentials noKey =
        ItauCredentials.parse(
            "{\"client_id\":\"c\",\"client_secret\":\"s\",\"pix_key\":\"60701190000104\"}"
                .getBytes());
    client().getCob(noKey, TXID);
    server.verify(
        getRequestedFor(urlEqualTo("/v2/cob/" + TXID))
            .withoutHeader("x-itau-apikey")
            .withHeader("Authorization", equalTo("Bearer tok")));
  }
  ...
  @Test
  void getCobCompletedCarriesPix() {
    server.stubFor(
  ...
  @Test
  void getCob404WithPixBodyIsEmpty() {
    server.stubFor(
        get("/v2/cob/" + TXID)
            .willReturn(
                aResponse()
  ...
  @Test
  void getCob404WithoutPixBodyIsAnError() {
    server.stubFor(
        get("/v2/cob/" + TXID)
            .willReturn(
                aResponse()
    ...
    assertThatThrownBy(() -> client().getCob(creds(), TXID))
  ...
  @Test
  void malformed2xxBodyIsUnknown() {
  ...
  @Test
  void problemWithoutTitleHasNoNullInTheMessage() {
    server.stubFor(
        get("/v2/cob/" + TXID)
            .willReturn(
                aResponse()
                    .withStatus(400)
                    .withBody(
                        "{\"type\":\"https://pix.bcb.gov.br/api/v2/error/CobConsultaInvalida\",\"detail\":\"txid invalido\"}")));
    assertThatThrownBy(() -> client().getCob(creds(), TXID))
        .isInstanceOfSatisfying(
            ProviderException.class,
            e -> {
  ...
  @Test
  void patchCobCancelSendsStatus() {
    server.stubFor(
    ...
    CobResponse r =
        client().patchCob(creds(), TXID, Map.of("status", "REMOVIDA_PELO_USUARIO_RECEBEDOR"));
    assertThat(r.status()).isEqualTo("REMOVIDA_PELO_USUARIO_RECEBEDOR");
  }

  @Test
  void patchCobOnConcludedIsInvalid() {
    server.stubFor(
        patch(urlEqualTo("/v2/cob/" + TXID))
            .willReturn(
                aResponse()
    ...
    assertThatThrownBy(
            () ->
                client()
                    .patchCob(creds(), TXID, Map.of("status", "REMOVIDA_PELO_USUARIO_RECEBEDOR")))
        .isInstanceOfSatisfying(
            ProviderException.class,
            e -> {
              assertThat(e.code()).isEqualTo(ProviderException.Code.INVALID);
  ...
  @Test
  void putDevolucaoReturnsProcessing() {
    server.stubFor(
    ...
    DevolucaoResponse r =
        client().putDevolucao(creds(), E2E, "123456", new DevolucaoRequest("100.00"));
    assertThat(r.status()).isEqualTo("EM_PROCESSAMENTO");
    assertThat(r.horario().liquidacao()).isNull();
  }

  @Test
  void getDevolucaoDone() {
    server.stubFor(
  ...
  @Test
  void listCobWindowAndPagination() {
    server.stubFor(
        get(urlPathEqualTo("/v2/cob"))
    ...
    CobList l =
        client()
            .listCob(
                ...
                Instant.parse("2020-04-02T10:00:00Z"),
                0,
                100);
  ...
  void socketTimeoutIsTimeout() {
    server.stubFor(
        get("/v2/cob/" + TXID)
            .willReturn(okJson(fixture("get_cob_200_active.json")).withFixedDelay(3000)));
    assertThatThrownBy(() -> client(Duration.ofSeconds(1)).getCob(creds(), TXID))
        .isInstanceOfSatisfying(
            ProviderException.class,
  ...
  @Test
  void serviceUnavailableIsUnavailable() {
   ...
   */
  @Test
  void productionShapeSendsApiKeyHeader() throws Exception {
    TestCertificates.Bundle certs = TestCertificates.generate();
    File serverKs = File.createTempFile("server", ".p12"),
        trustKs = File.createTempFile("trust", ".p12");
    try (var o = new FileOutputStream(serverKs)) {
      certs.serverKeyStore().store(o, certs.serverPassword());
    }
    try (var o = new FileOutputStream(trustKs)) {
      certs.caTrust().store(o, "changeit".toCharArray());
    }
    WireMockServer https =
        new WireMockServer(
            WireMockConfiguration.options()
                .dynamicHttpsPort()
                .httpDisabled(true)
                .keystorePath(serverKs.getAbsolutePath())
                .keystorePassword(new String(certs.serverPassword()))
                .keyManagerPassword(new String(certs.serverPassword()))
                .keystoreType("PKCS12")
                .trustStorePath(trustKs.getAbsolutePath())
    ...
    try {
      https.stubFor(
          post("/as/token.oauth2")
              .willReturn(okJson("{\"access_token\":\"prod-tok\",\"expires_in\":300}")));
      https.stubFor(get("/v2/cob/" + TXID).willReturn(okJson(fixture("get_cob_200_active.json"))));
      ItauCredentials full =
          ItauCredentials.parse(
              ("{\"client_id\":\"11111111-2222-3333-4444-555555555555\",\"client_secret\":\"s3cr3t\",\"x_itau_apikey\":\""
                      + APIKEY
                      + "\","
                      + "\"certificate_pem\":"
                      + ItauTokenClientMtlsTest.json(certs.clientCertPem())
                      + ",\"private_key_pem\":"
                      + ItauTokenClientMtlsTest.json(certs.clientKeyPem())
                      + ",\"pix_key\":\"60701190000104\"}")
                  .getBytes());
      ...
      https.verify(
          getRequestedFor(urlEqualTo("/v2/cob/" + TXID))
              .withHeader("x-itau-apikey", equalTo(APIKEY))
              .withHeader("Authorization", equalTo("Bearer prod-tok")));
    } finally {
      https.stop();
      serverKs.delete();
      trustKs.delete();
