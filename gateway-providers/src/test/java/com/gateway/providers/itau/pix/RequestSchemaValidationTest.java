 ...
 * {@code cpf} AND {@code cnpj} (Bacen's own spec has {@code oneOf PessoaFisica/PessoaJuridica},
 * each with one document), and the cpf/cnpj patterns keep JavaScript slashes ({@code
 * "/^\d{11}$/"}), so no real document can match. Validating against it verbatim would reject every
 * body with a payer. We validate against a copy with exactly those two fixes, and {@link
...
class RequestSchemaValidationTest {
  ...
  @Test
  void itauPessoaSchemaStillHasTheKnownDefects() throws Exception {
  ...
  @Test
  void thePatchedSchemaRejectsAPayerWithBothDocuments() throws Exception {
    var body =
        M2.readTree(
            "{\"calendario\":{\"expiracao\":3600},\"valor\":{\"original\":\"1.00\"},\"chave\":\"k\","
  ...
  @Test
  void createChargeBodyIsValid() throws Exception {
    CobRequest body =
        CobRequest.forCharge(
            Money.brl(15990), 3600, "60701190000104", "12345678909", "Jane Doe", "Order 8812");
    assertThat(validate("cobrancaImediataPutRequest", body)).isEmpty();
  }

  @Test
  void createChargeWithCnpjPayerIsValid() throws Exception {
    CobRequest body =
        CobRequest.forCharge(
            Money.brl(15990),
            3600,
  ...
  @Test
  void createChargeWithoutPayerIsValid() throws Exception {
    CobRequest body =
        CobRequest.forCharge(Money.brl(100), 3600, "60701190000104", null, null, null);
    assertThat(validate("cobrancaImediataPutRequest", body)).isEmpty();
  }

  @Test
  void refundBodyIsValid() throws Exception {
    assertThat(
            validate(
                "devolucaoPutRequest", new DevolucaoRequest(PixAmounts.toItau(Money.brl(1000)))))
        .isEmpty();
  }
