...
public class ItauTokenClientMtlsTest {
  ...
  static void start() throws Exception {
    ...
    serverKs = File.createTempFile("server", ".p12");
  ...
  ItauCredentials creds() {
    return ItauCredentials.parse(
        ("{\"client_id\":\"11111111-2222-3333-4444-555555555555\",\"client_secret\":\"s3cr3t\",\"x_itau_apikey\":\"aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee\","
                + "\"certificate_pem\":"
                + json(certs.clientCertPem())
                + ",\"private_key_pem\":"
                + json(certs.clientKeyPem())
                + ",\"pix_key\":\"60701190000104\"}")
            .getBytes());
  }

  // BouncyCastle's PEMWriter emits \r\n on Windows; unescaped \r is an illegal control char in
  // JSON.
  ...
  URI tokenUrl() {
    return URI.create("https://localhost:" + server.httpsPort() + "/as/token.oauth2");
  }

  @Test
  void postsClientCredentialsOverMtlsAndCachesUntilNearExpiry() {
    server.stubFor(
        post("/as/token.oauth2")
            .withHeader("Content-Type", containing("application/x-www-form-urlencoded"))
            .withRequestBody(containing("grant_type=client_credentials"))
            .withRequestBody(containing("client_id=11111111-2222-3333-4444-555555555555"))
  ...
  @Test
  void refreshesSixtySecondsBeforeExpiry() {
    server.stubFor(
        post("/as/token.oauth2")
            .willReturn(okJson("{\"access_token\":\"tok-a\",\"expires_in\":300}")));
   ...
   */
  @Test
  void serverRejectsConnectionsWithoutClientCertificate() throws Exception {
    TestCertificates.Bundle other = TestCertificates.generate();
    var plain =
        java.net.http.HttpClient.newBuilder()
            .sslContext(
                PemKeyStores.mutualTls(
                    other.clientCertPem(), other.clientKeyPem(), certs.caTrust()))
            .build();
    // a cert from ANOTHER CA is not trusted by the server
    assertThatThrownBy(
            () ->
                plain.send(
                    java.net.http.HttpRequest.newBuilder(tokenUrl())
  ...
  @Test
  void sandboxTokenWithoutClientCertificate() throws Exception {
  ...
  @Test
  void mutualTlsWithSandboxShapedCredentialsFails() {
    ItauCredentials sandboxCreds =
        ItauCredentials.parse(
            "{\"client_id\":\"sandbox-client\",\"client_secret\":\"sandbox-secret\",\"pix_key\":\"60701190000104\"}"
                .getBytes());
