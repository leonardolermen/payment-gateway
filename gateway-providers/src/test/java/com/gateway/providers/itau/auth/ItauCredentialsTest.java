...
class ItauCredentialsTest {
  ...
  void parsesAllSixFields() {
    ...
    assertThat(c.toString())
        ...
        .doesNotContain("MIIE")
        .doesNotContain("aaaaaaaa-bbbb")
        .contains("apiKey=***");
  }

  @Test
  void missingFieldNamesTheField() {
  ...
  @Test
  void apiKeyMustMatchItauRegex() {
    assertThatThrownBy(
            () ->
                ItauCredentials.parse(
                    JSON.replace("aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee", "not-a-uuid").getBytes()))
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining("x_itau_apikey")
        .hasMessageNotContaining("not-a-uuid");
  }

  @Test
  void sandboxShapeHasNoCertificate() {
    ItauCredentials c =
        ItauCredentials.parse(
    ...
    assertThatThrownBy(c::requireProductionShape)
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining("certificate_pem");
    assertThatThrownBy(
            () ->
                ItauCredentials.parse(
                    "{\"client_id\":\"x\",\"client_secret\":\"y\",\"pix_key\":\"k\",\"certificate_pem\":\"c\"}"
                        .getBytes()))
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining("private_key_pem");
  }

  @Test
  void fingerprintChangesWhenAnyFieldChanges() {
    ItauCredentials a = ItauCredentials.parse(JSON.getBytes());
    assertThat(a.fingerprint())
        .hasSize(64)
        .isEqualTo(ItauCredentials.parse(JSON.getBytes()).fingerprint());
    for (String[] change :
        new String[][] {
          {"s3cr3t", "other"},
          {"MIIE", "MIIF"},
          {"MIIB", "MIIC"},
          {"60701190000104", "60701190000105"},
          {"11111111-2222", "11111111-9999"},
          {"aaaaaaaa-bbbb", "aaaaaaaa-ffff"}
        }) {
  ...
  @Test
  void blankPrivateKeyCountsAsMissing() {
    assertThatThrownBy(
            () ->
                ItauCredentials.parse(
                    JSON.replace(
                        ...
                        .getBytes()))
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining("without private_key_pem");
  }

  static final String WITH_BOLETO =
      "{\"client_id\":\"sbx-id\",\"client_secret\":\"sbx-secret\",\"pix_key\":\"60701190000104\","
          + "\"beneficiary_id\":\"150000052061\",\"wallet_code\":\"109\",\"species_code\":\"01\"}";

  @Test
  void parsesTheBoletoFields() {
  ...
  @Test
  void walletAndSpeciesDefaultWhenAbsent() {
    ItauCredentials c =
        ItauCredentials.parse(
            WITH_BOLETO.replace(",\"wallet_code\":\"109\",\"species_code\":\"01\"", "").getBytes());
    assertThat(c.walletCode()).isEqualTo("109");
    assertThat(c.speciesCode()).isEqualTo("01");
  }

  @Test
  void pixOnlyCredentialHasNoBeneficiaryAndSaysSo() {
    ItauCredentials c =
        ItauCredentials.parse(
            "{\"client_id\":\"sbx-id\",\"client_secret\":\"sbx-secret\",\"pix_key\":\"60701190000104\"}"
                .getBytes());
    assertThat(c.hasBeneficiary()).isFalse();
    assertThatThrownBy(c::requireBoletoShape)
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining("beneficiary_id");
  }

  @Test
  void boletoFieldsAreValidatedByRegex() {
    assertThatThrownBy(
            () ->
                ItauCredentials.parse(WITH_BOLETO.replace("150000052061", "1500000520").getBytes()))
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining("beneficiary_id");
    assertThatThrownBy(
            () -> ItauCredentials.parse(WITH_BOLETO.replace("\"109\"", "\"1090\"").getBytes()))
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining("wallet_code");
    assertThatThrownBy(
            () -> ItauCredentials.parse(WITH_BOLETO.replace("\"01\"", "\"1\"").getBytes()))
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining("species_code");
  }

  @Test
  void beneficiaryChangesTheFingerprint() {
    assertThat(ItauCredentials.parse(WITH_BOLETO.getBytes()).fingerprint())
        .isNotEqualTo(
            ItauCredentials.parse(WITH_BOLETO.replace("150000052061", "150000052062").getBytes())
                .fingerprint());
  }
}
