...
/**
 * Itaú "Consulta de detalhe" (boletoscash v2): the only way to learn that a Bolecode was paid by
 * barcode (no boleto webhook in scope, spec §8).
 */
class BoletoQueryClient {
  ...
  Optional<BoletoQueryItem> find(ItauCredentials c, String nossoNumero) {
    ...
    return body.data().stream()
        .filter(item -> item.individual(nossoNumero).isPresent())
        .findFirst();
  }
}
