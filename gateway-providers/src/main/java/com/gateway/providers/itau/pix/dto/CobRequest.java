...
public record CobRequest(
  ...
  /**
   * {@code solicitacaoPagador} is capped at 140 chars by the schema; longer descriptions are cut,
   * not rejected.
   */
  public static CobRequest forCharge(
      Money amount,
    ...
    if (payerDocument != null && payerName != null) {
      ...
      devedor =
          digits.length() == 14
              ? new Devedor(null, digits, payerName)
              : new Devedor(digits, null, payerName);
    }

    return new CobRequest(
        new Calendario(expiresInSeconds),
        devedor,
        new Valor(PixAmounts.toItau(amount)),
        pixKey,
        description == null ? null : description.substring(0, Math.min(140, description.length())),
        null);
  }
}
