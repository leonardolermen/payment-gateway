...
/**
 * The bank's dates carry no zone; a boleto day is a São Paulo day (spec 2026-09-25, global
 * constraint).
 */
public final class ItauDates {
  ...
  /**
   * {@code data_hora_inclusao_pagamento} first (ISO instant or ISO local date-time read as São
   * Paulo); {@code data_inclusao_pagamento} as start of that day; null when neither parses.
   */
  public static Instant paidAt(String dateTime, String date) {
    if (dateTime != null) {
      ...
      try {
        return LocalDateTime.parse(dateTime).atZone(SAO_PAULO).toInstant();
      } catch (DateTimeParseException ignored) {
